...
foreach($categories as $category) {
    echo "<tr><form action='/change_category.php' id='" .  $category->id . "' method='get'>
        <td><input type='text' form='" .  $category->id . "' name ='id' value='" . $category->id . "' readonly></td>
        <td><input type='text' form='" .  $category->id . "'value='" . $category->name . "' name='name'></td>
        <td><input type='submit' form='" .  $category->id . "'name='action' value='change'>
        <td><input type='submit' form='" .  $category->id . "' name='action' value='delete'></td></form></tr>"; 
    }
echo "<tr><form action='/change_category.php' id='new_cat' method='get'>
    <td><input type='text' form='new_cat' name ='id' value='auto_increment' readonly></td>
    <td><input type='text' form='new_cat' value='' placeholer='Kategoriename' name='name'></td>
...
echo "<table border=1><tr><th>ID</th><th>Name</th><th>Preis in €</th><th>Kategorie</th></tr>";
foreach($articles as $article) {
    echo "<tr><form action='/change_article.php' id='" .  $article->id . "' method='get'>
        <td><input type='text' name ='id' value='" . $article->id . "' readonly></td>
        <td><input type='text' form='" .  $article->id . "' value='" . $article->name . "' name='name'></td>
        <td><input type='number' form='" .  $article->id . "' min=0 step=0.01 value='" . $article->price . "' name='price'></td>
...
echo "<tr><form action='/change_article.php' id='new_art' method='get'>
    <td><input type='text' form='new_art' name='id' value='auto_increment' readonly></td>
    <td><input type='text' form='new_art' value='' placeholer='Artikelname' name='name'></td>
    <td><input type='text' form='new_art' value='' placeholer='Preis' name='price'></td>
