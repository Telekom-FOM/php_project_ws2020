...
{
    $mysqli = db_connect();
    $sql = "SELECT article_nr, article.name AS art_name, price, category, article_category.name AS cat_name FROM article JOIN article_category on category=article_category.id where article_nr=?";
    $con = $mysqli->prepare($sql);
    ...
    else
    {
        while ($row = $res->fetch_assoc())
        {
            ...
            $article->category_name = $row['cat_name'];
        }
    }
    return $article;
}
...
{
    $mysqli = db_connect();
    $sql = "SELECT article_nr, article.name AS art_name, price, category, article_category.name AS cat_name FROM article JOIN article_category on category=article_category.id";
    $res = $mysqli->query($sql);
    if ($res->num_rows == 0)
    {
        return false;
